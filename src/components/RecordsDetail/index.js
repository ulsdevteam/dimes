...
/**test */
 ...
 const handleSubquery = (e)=>{
  setsubquery(e.target.value);
  if(subquery){
    setSearchParams(searchParams=>{
      searchParams.set("query", subquery);
      for (const entry of searchParams.entries()) {
        console.log(entry)
      }
      console.log("Check" ,props.params.query);
      return searchParams;
    });
   }
 }
 ...
 useEffect(()=>{
  console.log("PreviouQuery: %s  SubqueryValue: %s" ,props.params.query, subquery);
 })

//navigate to new path
const navigate = useNavigate();
const handleSubmitSubquery = (e)=>{
  const path = window.location.pathname;  //new query
  const tempsearch = window.location.search;
  testurl =window.location.href;
  navigate(path+tempsearch); 
} 
  ...
  <div className={classnames('records__detail', {'hidden': props.isContentShown})}>
    ...
    <form onSubmit={handleSubmitSubquery}>
      <input 
        className="subquery-search" 
        type='text' 
        placeholder='refine search'
        value={subquery ?? undefined} 
        onChange={handleSubquery}/>
      <button type='submit' className='subquery-clear' >Clear </button>
    </form> 
